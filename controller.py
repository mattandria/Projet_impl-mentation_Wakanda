...
class Controller(Thread):
        ...
        def __init__(self, periodPWM, periodEncoders, pinPWM1, pinDir1, pinPWM2, pinDir2):
                ...
                #Encodeurs
                self.encoder1 = eQEP("/sys/devices/ocp.3/48304000.epwmss/48304180.eqep", eQ$
                self.encoder1.set_period(periodEncoders)
                self.encoder2 = eQEP("/sys/devices/ocp.3/48300000.epwmss/48300180.eqep", eQ$
                ...
                self.PWM = PWM
                self.PWM.start(self.pinPWM1, 0, periodPWM)
                self.PWM.start(self.pinPWM2, 0, periodPWM)
        ...
        def regulateCmd(self, motor, cmd):
                ...
                if(cmd > 0):
                        ...
                        if(cmd > 100):  #le duty_cycle ne peut pas depasser la valeur de la$
                                cmd = 100
                elif(cmd < 0):
